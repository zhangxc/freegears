...
PKG_PATCHES	+= iproute2-2.6.31-qos.patch
...
build: $(PKG) $(PKG)/Makefile
	make -C $(PKG) CC=$(CC) PREFIX=$(PKG_INS_DIR) KERNEL_INCLUDE=$(CFG_KERNEL_DIR)/include
...
#   $(CFG_TARGET_ROOT)
install:
	make -C $(PKG) install CC=$(CC) PREFIX=$(PKG_INS_DIR)  KERNEL_INCLUDE=$(CFG_KERNEL_DIR)/include
	$(STRIP) -s $(PKG_INS_DIR)/sbin/{ip,rtmon,tc}
	$(RM) -r $(PKG_INS_DIR)/{share,usr,var}
