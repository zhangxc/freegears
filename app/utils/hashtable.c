 ...
 * Date:   August, 1998
 * Azalea Revision:     02/14/2008 by Wilson (add iterator)
 *
 * Released to the public domain.
 *
 *--------------------------------------------------------------------------
 * $Id: hashtable.c,v 1.1 2009/03/24 06:18:33 qzhou Exp $
\*--------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------*\
 *  NAME:
 *      HashTableIterator() - apply function on all nodes in a HashTable
 *  DESCRIPTION:
 *      Go through all nodes in an existing HashTable.
 *  EFFICIENCY:
 *      O(n)
 *  ARGUMENTS:
 *      hashTable    - the HashTable to parse 
 *  RETURNS:
 *      <nothing>
\*--------------------------------------------------------------------------*/

void HashTableIterator (HashTable *hashTable, void *usr_data,
            void (*func)(void *value, void *usr_data))
{
    int i;
...
/*--------------------------------------------------------------------------*\
 *  NAME:
 *      HashTableIterator_plus() - apply function on all nodes in a HashTable
 *  DESCRIPTION:
 *      Go through all nodes in an existing HashTable.
 *  Note:
 *      Intend for node non-delete action. OK to modify internal value but 
 *      !!Don't delete the node in user function!! 
 *
\*--------------------------------------------------------------------------*/
void HashTableIterator_plus(HashTable *hashTable, void *usr_data,
            void (*func)(const void * key,void *value, void *usr_data))
{
	int i;
    for (i=0; i<hashTable->numOfBuckets; i++) {
        KeyValuePair *pair = hashTable->bucketArray[i];
        while (pair != NULL) {
            KeyValuePair *nextPair = pair->next;
        func(pair->key,pair->value, usr_data);
            pair = nextPair;
        }
    }
}
